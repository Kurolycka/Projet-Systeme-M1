...
if [ "$1" == "-h" ] || [ "$1" == "-help" ]; then #Affichage de l'help de base, -h et -help.
    ...
    \n Afin de ne pas noyer l'utilisateur sous les informations l'aide est divisée en catégories avec plusieurs options. Choisissez celle correspondant aux informations que vous recherchez. 
    ...
    Informations plus détaillées conçernant ce programme sur le git correspondant : https://github.com/Kurolycka/Projet-Systeme-M1.git"
    ...
    \e[1;36;4mNuméro de la colonne contenant les longueurs d'onde (colonne_longueur_onde) :\e[0m  Doit être un nombre entier strictement positif et ne doit pas être supérieur au nombre de colonnes contenues dans votre fichier.
    La première colonne de votre fichier a le numéro 1 et non le numéro 0. 
    
    \e[1;36;4mNuméro de la colonne contenant les intensités (colonne_intensitees) :\e[0m  Doit être un nombre entier strictement positif et ne doit pas être supérieur au nombre de colonnes contenues dans votre fichier.
    La première colonne de votre fichier a le numéro 1 et non le numéro 0. 
    ...
    \e[1;36;4mUnité de vos longueurs d'onde (unite_long_onde) :\e[0m  Il n'y a pas de conditions sur ce paramètre. Il servira à spécifier l'abscisse de votre graphe.
    ...
    Renseignez la dans l'unité que vous avez choisie pour le paramètre 'Unité de vos longueurs d'ondes'.
    ...
    Renseignez la dans l'unité que vous avez choisit pour le paramètre 'Unité de vos longueurs d'onde'.

    \e[1;36;4m Le séparateur (sep) :\e[0m La chaîne de caractère entre deux de vos valeurs numériques dans votre fichier.

    \e[1;32mPour certains paramètres vous aurez de nouvelles chances de rentrer le bon paramètre sans quitter le programme si vous vous êtes trompé.\e[0m
    "
    exit #Sortie du programme..
    ...
    Tout d'abord vos données de longueurs d'onde et leurs intensités correspondantes sont séparées en intervalles selon le pas choisi. Si par exemple vous avez des données entre 300 et 400 nm et que vous choisissez un pas de 10 vous aurez des intervalles tels que [300,310[;[310, 320[ ... [380,390[;[390,400].
    ...
    Ce qui sera au final affiché sur le graphe seront les intervalles dans lesquels sont contenues vos valeurs de début et de fin d'intervalle pour le graphe ainsi que tous les intervalles entre.
    ...
    \e[1;36;4mCommentaires :\e[0m Il vous est possible de mettre des commentaires dans votre fichier qui seront affichés lors de l'exécution du programme. Cependant ces derniers doivent obligatoirement être précédés par un #.
...
read -p "Entrez le numéro de la colonne qui contient les intensités: " colonne_intensitees #Je récupère la colonne des intensitées.
